...
void main() {
  ...
  testWidgets('Flow 1: Lead to Order E2E Test', (WidgetTester tester) async {
    ...
    int waitRetries = 0;
    while(find.text('Save & Initialize').evaluate().isEmpty && find.text('Session Locked').evaluate().isEmpty && waitRetries < 50) {
    ...
    int homeRetries = 0;
    while (find.text('Command Center').evaluate().isEmpty && homeRetries < 20) {
    ...
    // 4. Navigate to CRM tab (lands directly on the Kanban leads board)
    ...
    // Tap the Kanban card containing our test client to open Lead Details
    await tester.tap(find.textContaining(uniqueLeadName).first);
    await tester.pump(const Duration(seconds: 2));
    ...
    // 8. Create Quotation (opens the full-screen quotation builder)
    await tester.tap(find.text('Create Quotation'));
    await tester.pump(const Duration(seconds: 2));

    // Fill out the first line item + tax rate
    ...
    await tester.tap(find.text('Generate & Save'));
    await tester.pump(const Duration(seconds: 2));
    ...
    // Pop back from Lead Details to the CRM tab root, then switch to the
    // Logistics tab directly.
